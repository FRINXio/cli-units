...
public class ClientGroupConfigWriterTest {
    @Mock
    private Cli cli;
    @Mock
    private WriteContext writeContext;
    private ClientGroupConfigWriter target;
    private ArgumentCaptor<Command> response = ArgumentCaptor.forClass(Command.class);

    private static final String WRITE_INPUT = "/configure\n"
        + "ipsec\n"
        + "client-db \"Tunnel-Group-001\" create\n"
        + "exit all\n";
    private static final String DELETE_INPUT = "/configure\n"
        + "ipsec\n"
        + "no client-db \"Tunnel-Group-001\"\n"
        + "exit all\n";
    ...
    public void setUp() throws Exception {
        ...
        target = new ClientGroupConfigWriter(cli);
    }
