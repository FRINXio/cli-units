...
public class ClientIdentificationConfigWriter implements CliWriter<Config> {
    ...
    private static final String CREATE_TEMPLATE = "/configure\n"
        ...
        + "{% if ($config.idi_host.domain_name) %}"
        + "idi string-type fqdn string-value \"{$config.idi_host.domain_name.value}\"\n"
        + "{% else %}"
        + "no idi\n"
        + "{% endif %}"
        + "{% if ($config.peer_prefix.ipv6_prefix) %}"
        + "peer-ip-prefix {$config.peer_prefix.ipv6_prefix.value}\n"
        + "{% else %}"
        + "no peer-ip-prefix\n"
        + "{% endif %}"
        + "exit all";
