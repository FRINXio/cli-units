...
class SubinterfaceConfigWriterTest {
    ...
    private static final String WRITE_INPUT = """
            set interfaces ge-0/0/4 unit 1 description TEST-ge-0/0/4

            """;

    private static final String UPDATE_INPUT = """
            set interfaces ge-0/0/4 unit 1 description TEST-ge-0/0/4_NEW
    ...
    private static final String UPDATE_TO_NULL = """
            delete interfaces ge-0/0/4 unit 1 description
    ...
    void writeNull() throws Exception {
        ...
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(DELETE_DESCR_INPUT, response.getValue().getContent());
    }
    ...
    void updateToNull() throws Exception {
        ...
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(UPDATE_TO_NULL, response.getValue().getContent());
    }
