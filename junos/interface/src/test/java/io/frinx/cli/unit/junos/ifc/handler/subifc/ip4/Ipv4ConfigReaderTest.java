...
class Ipv4ConfigReaderTest {
    ...
    void testParseConfigAddress() {
        ...
        assertEquals(AbstractIpv4ConfigReaderTest.buildData("10.11.12.13", "16"), actual.build());
    }

    @Test
    void testParseConfigAddressSubIf() {
        final ConfigBuilder actual = new ConfigBuilder();
        new Ipv4ConfigReader(Mockito.mock(Cli.class)).parseAddressConfig(actual, OUTPUT_SINGLE_SUB_IFACE, null);
        assertEquals(AbstractIpv4ConfigReaderTest.buildData("10.11.12.23", "16"), actual.build());
    }
}
