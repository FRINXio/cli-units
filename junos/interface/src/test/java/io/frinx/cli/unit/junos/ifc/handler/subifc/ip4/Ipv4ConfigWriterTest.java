...
class Ipv4ConfigWriterTest {
    ...
    void testUpdate() throws Exception {
        ...
        assertEquals(DELETE_INPUT, response.getAllValues().get(0).getContent());
        assertEquals(UPDATE_INPUT, response.getAllValues().get(1).getContent());
    }

    @Test
    void testWriteSubif() throws Exception {
        writer.writeCurrentAttributes(idSubif, dataSubif, context);

        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(WRITE_INPUT_SUBIF, response.getValue().getContent());
    }

    @Test
    void testDeleteSubif() throws Exception {
        writer.deleteCurrentAttributes(idSubif, dataSubif, context);

        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(DELETE_INPUT_SUBIF, response.getValue().getContent());
    }

    @Test
    void testUpdateSubif() throws Exception {
        ...
        Mockito.verify(cli, Mockito.times(2)).executeAndRead(response.capture());
        assertEquals(DELETE_INPUT_SUBIF, response.getAllValues().get(0).getContent());
        assertEquals(UPDATE_INPUT_SUBIF, response.getAllValues().get(1).getContent());
    }
}