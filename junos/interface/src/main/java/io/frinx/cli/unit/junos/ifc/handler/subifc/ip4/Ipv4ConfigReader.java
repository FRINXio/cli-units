...
public class Ipv4ConfigReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    public void readCurrentAttributes(@Nonnull InstanceIdentifier<Config> id, @Nonnull ConfigBuilder configBuilder,
        ...
        parseAddressConfig(configBuilder, blockingRead(
            String.format(Ipv4AddressReader.SH_RUN_INT_IP, ifcKey.getName(), subKey.getIndex()), cli, id, readContext));
    }

    @VisibleForTesting
    static void parseAddressConfig(ConfigBuilder configBuilder, String output) {
        ParsingUtils
            .parseField(output, Ipv4AddressReader.INTERFACE_IP_LINE::matcher,
                m -> new Ipv4AddressNoZone(m.group("address")), configBuilder::setIp);
