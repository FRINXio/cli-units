...
public class Ipv4AddressReader implements CliConfigListReader<Address, AddressKey, AddressBuilder> {
    ...
    public static final String SH_RUN_INT_IP =
    ...
    static final Pattern INTERFACE_IP_LINE = Pattern
        .compile("set interfaces (?<ifcId>.+) unit (?<subifcIndex>[0-9]+) family "
            + "inet address (?<address>[^/]+)/(?<prefix>[0-9]+)");
    ...
    public List<AddressKey> getAllIds(@Nonnull InstanceIdentifier<Address> instanceIdentifier,
        ...
        InterfaceKey ifcKey = instanceIdentifier.firstKeyOf(Interface.class);
        SubinterfaceKey subKey = instanceIdentifier.firstKeyOf(Subinterface.class);

        return parseAddressIds(blockingRead(String.format(SH_RUN_INT_IP, ifcKey.getName(), subKey.getIndex()), cli,
            instanceIdentifier, readContext));
    }

    @VisibleForTesting
    public static List<AddressKey> parseAddressIds(String output) {
        return ParsingUtils.parseFields(output, 0, INTERFACE_IP_LINE::matcher, m -> m.group("address"),
            addr -> new AddressKey(new Ipv4AddressNoZone(addr)));
    }

    @Override
    public void merge(@Nonnull Builder<? extends DataObject> builder, @Nonnull List<Address> list) {
