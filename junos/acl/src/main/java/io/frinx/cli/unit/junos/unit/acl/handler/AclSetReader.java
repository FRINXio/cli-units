...
public class AclSetReader implements CliConfigListReader<AclSet, AclSetKey, AclSetBuilder> {
    ...
    private static final String SH_ACCESS_LISTS = "show configuration firewall | display set";
    private static final Pattern ACL_LINE = Pattern.compile("set firewall family (?<type>.+) filter (?<name>\\S+).*");
    ...
    @NotNull
    @Override
    public List<AclSetKey> getAllIds(@NotNull InstanceIdentifier<AclSet> id, @NotNull ReadContext context)
            throws ReadFailedException {
        return parseAccessLists(blockingRead(SH_ACCESS_LISTS, cli, id, context));
    }

    @VisibleForTesting
    List<AclSetKey> parseAccessLists(String output) {
    ...
    private AclEntries getAclEntries(final InstanceIdentifier<AclSet> id, final ReadContext ctx)
            throws ReadFailedException {
