...
class AreaInterfaceReaderTest {
    ...
    private static final String OUTPUT_AREAS = """
            ...
            set routing-instances APTN protocols ospf area 10.51.246.21 interface xe-0/0/34.0 bfd-liveness-detection\
             multiplier 3\r
            set routing-instances APTN protocols ospf area 10.51.246.21 interface xe-0/0/35.0 disable\r
            set routing-instances APTN protocols ospf area 10.51.246.21 interface xe-0/0/36.0 disable\r
            """;
    ...
    void testGetAllIds_001() throws Exception {
        ...
        assertThat(result, CoreMatchers.equalTo(EXPECTED_INTERFACE_KEYS));
    ...
    @Test
    void testReadCurrentAttributesForType() throws Exception {
        final InstanceIdentifier<Interface> iid = IIDs.NETWORKINSTANCES
                .child(NetworkInstance.class, new NetworkInstanceKey("APTN"))
                .child(Protocols.class)
                .child(Protocol.class, new ProtocolKey(OSPF.class, OspfProtocolReader.OSPF_NAME))
        ...
        assertEquals("xe-0/0/34.0", builder.getId());
    }
}
