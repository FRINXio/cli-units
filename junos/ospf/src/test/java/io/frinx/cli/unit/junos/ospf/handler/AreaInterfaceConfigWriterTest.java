...
class AreaInterfaceConfigWriterTest {
    ...
    @Test
    void testwriteCurrentAttributesForType() throws Exception {
        ...
        assertThat(commands.getAllValues().size(), CoreMatchers.is(5));
        ...
        assertThat(commands.getAllValues().get(4).getContent(), CoreMatchers.equalTo(
                "set routing-instances APTN protocols ospf area 10.51.246.21 interface xe-0/0/34.0 priority 11\n"));
    }

    @Test
    void testDeleteCurrentAttributesForType() throws Exception {
        final Config data = Mockito.mock(Config.class);
        final ArgumentCaptor<Command> commands = ArgumentCaptor.forClass(Command.class);
        ...
        assertThat(commands.getValue().getContent(), CoreMatchers.equalTo(
