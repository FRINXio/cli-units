...
public class ActionsConfigReaderTest {
    ...
    private ActionsConfigReader target;

    @Before
    public void setUp() throws Exception {
        MockitoAnnotations.initMocks(this);
        target = Mockito.spy(new ActionsConfigReader(cli));
    }
    ...
    public void testReadCurrentAttributes() throws Exception {
        ...
        Mockito.doReturn("accept;").when(target).blockingRead(Mockito.anyString(), Mockito.eq(cli),
                Mockito.eq(instanceIdentifier), Mockito.eq(readContext));
        // test
        target.readCurrentAttributes(instanceIdentifier, builder, readContext);
        // verify
        Assert.assertEquals(PolicyResultType.ACCEPTROUTE, builder.getPolicyResult());
    }
}