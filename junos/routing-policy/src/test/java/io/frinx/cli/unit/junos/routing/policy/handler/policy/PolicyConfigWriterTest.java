...
class PolicyConfigWriterTest implements CliFormatter {
    ...
    void testWriteCurrentAttributes() throws Exception {
        ...
        assertEquals(f(PolicyConfigWriter.SH_WRITE_POLICY_STATEMENT, PolicyConfigWriter.VERB_SET,
    ...
    void testDeleteCurrentAttributes_001() throws Exception {
        ...
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(f(PolicyConfigWriter.SH_WRITE_POLICY_STATEMENT, PolicyConfigWriter.VERB_DEL,
                POLICY_DEFINITION_NAME.concat("\n")), response.getValue().getContent());
    }
}
