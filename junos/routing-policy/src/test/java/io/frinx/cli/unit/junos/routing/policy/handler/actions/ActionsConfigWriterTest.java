...
class ActionsConfigWriterTest implements CliFormatter {
    ...
    void testWriteCurrentAttributes() throws Exception {
        ...
        assertEquals(f(ActionsConfigWriter.SH_WRITE_POLICY_RESULT, ActionsConfigWriter.VERB_SET,
    ...
    void testDeleteCurrentAttributes_001() throws Exception {
        ...
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(f(ActionsConfigWriter.SH_WRITE_POLICY_RESULT, ActionsConfigWriter.VERB_DEL,
                POLICY_DEFINITION_NAME, STATEMENT_NAME, "accept\n"), response.getValue().getContent());
    }
}
