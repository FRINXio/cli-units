...
public class L3VrfReaderTest {
    ...
    public void setUp() throws Exception {
        ...
        target = Mockito.spy(new L3VrfReader(cli));
    }
    ...
    public void testGetAllIds_001() throws Exception {
        ...
        List<NetworkInstanceKey> result = target.getAllIds(iid, readContext);

        Assert.assertThat(result, CoreMatchers.equalTo(EXPECTED_VRF_KEYS));

        Mockito.verify(target, Mockito.times(1)).blockingRead(
            L3VrfReader.DISPLAY_CONF_VRF,
            cli,
            iid,
            readContext);
    }
