...
public class CerCliInitializerUnit extends AbstractUnit {

    private static final Logger LOG = LoggerFactory.getLogger(CerCliInitializerUnit.class);

    public CerCliInitializerUnit(@NotNull final TranslationUnitCollector registry) {
        super(registry);
    }
    ...
    protected String getUnitName() {
        return "CER cli init (FRINX) translate unit";
    }
    ...
    public SessionInitializationStrategy getInitializer(@NotNull final RemoteDeviceId id,
        ...
        return new CerCliInitializer(cliNodeConfiguration, id);
    }
    ...
    public Set<Pattern> getErrorPatterns() {
        return Sets.newLinkedHashSet(Arrays.asList(
                Pattern.compile("(^|\\n)Error: Bad command.*", Pattern.DOTALL),
                Pattern.compile("(^|\\n)Error: Enter \\S+ \\? to obtain help.*", Pattern.DOTALL),
                Pattern.compile("(^|\\n)Error: Ambiguous parameter.*", Pattern.DOTALL),
                Pattern.compile("(^|\\n)Error: Missing parameter.*", Pattern.DOTALL),
                Pattern.compile("(^|\\n)Error: Enter \\S+( \\S+)+ \\? to obtain help.*", Pattern.DOTALL),
                Pattern.compile("(^|\\n)Error: .*", Pattern.DOTALL)
        ));
    }
    ...
    public CliFlavour getCliFlavour() {
        return new CliFlavour(
                Pattern.compile("^(do )?sho?w? runn?i?n?g?-?c?o?n?f?i?g?"),
                " ",
                "",
                "!",
                Pattern.compile("\\|"),
                ...
                null,
                "show running-config",
                ImmutableMap.of("banner login","/end"),
                "interface cable-upstream");
    }

    /**
     * Initialize CER CLI session to be usable by various CRUD and RPC handlers.
     */
    public static class CerCliInitializer implements SessionInitializationStrategy {
        ...
        public CerCliInitializer(CliNode context, RemoteDeviceId id) {
