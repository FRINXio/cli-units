...
public class SubinterfaceConfigReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    private static CerIfAggregateSubifExtension.LevelType convertIsisLevelFromString(String level) {
        ...
        return switch (level) {
            case "level-1" -> CerIfAggregateSubifExtension.LevelType.LEVEL1;
            case "level-2" -> CerIfAggregateSubifExtension.LevelType.LEVEL2;
            case "level-1-2" -> CerIfAggregateSubifExtension.LevelType.LEVEL12;
            default -> throw new IllegalArgumentException("Unknown isis level " + level);
        };
    }
