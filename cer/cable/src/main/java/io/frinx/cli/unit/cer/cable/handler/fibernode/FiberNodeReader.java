...
public final class FiberNodeReader implements CliConfigListReader<FiberNode, FiberNodeKey, FiberNodeBuilder> {

    private static final String SH_CABLES = "show running-config | include cable fiber-node";
    ...
    @NotNull
    @Override
    public List<FiberNodeKey> getAllIds(@NotNull InstanceIdentifier<FiberNode> instanceIdentifier,
                                        @NotNull ReadContext readContext) throws ReadFailedException {
        final String output = blockingRead(SH_CABLES, cli, instanceIdentifier, readContext);
        return getFiberNodeKeys(output);
    }

    @VisibleForTesting
    public static List<FiberNodeKey> getFiberNodeKeys(String output) {
        return ParsingUtils.parseFields(output, 0, CABLE_LINE::matcher,
            matcher -> matcher.group("name"), FiberNodeKey::new);
    }

    @Override
    public void readCurrentAttributes(@NotNull InstanceIdentifier<FiberNode> instanceIdentifier,
                                      @NotNull FiberNodeBuilder schedulerPolicyBuilder,
