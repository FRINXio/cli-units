...
public class AclSetConfigWriter implements CliListWriter<AclSet, AclSetKey> {
    ...
    private static final String ACL_IPV4 = "configure\n"
            + "access-list ipv4 {$aclName}\n"
            + "end\n";

    private static final String ACL_IPV4_DELETE = "configure\n"
            + "no access-list ip {$aclName}\n"
            + "end\n";

    @Override
    public void writeCurrentAttributes(@Nonnull InstanceIdentifier<AclSet> instanceIdentifier,
    ...
    @Override
    public void deleteCurrentAttributes(@Nonnull InstanceIdentifier<AclSet> instanceIdentifier,
                                        @Nonnull AclSet aclSet,
                                        @Nonnull WriteContext writeContext) throws WriteFailedException {
        AclSetKey aclSetKey = instanceIdentifier.firstKeyOf(AclSet.class);
        String aclName = aclSetKey.getName();
