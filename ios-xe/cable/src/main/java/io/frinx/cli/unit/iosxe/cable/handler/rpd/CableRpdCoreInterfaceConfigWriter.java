...
public class CableRpdCoreInterfaceConfigWriter implements CliWriter<CoreInterface> {
    ...
    private static final String WRITE_TEMPLATE = """
            ...
            {% endif %}{% if ($nw_delay) %}network-delay {$nw_delay}
            {% endif %}{% loop in $rpdDs as $ds%}rpd-ds {$ds.id} {$ds.cable_controller} profile {$ds.profile}
            {% endloop %}{% loop in $rpdUs as $us%}rpd-us {$us.id} {$us.cable_controller} profile {$us.profile}
            {% endloop %}end""";
    ...
    private static final String UPDATE_TEMPLATE = """
            ...
            {% if ($principal == TRUE) %}principal
            {% else %}no principal
            {% endif %}{% if ($nw_delay) %}no network-delay
            network-delay {$nw_delay}
            {% endif %}{% loop in $rpdDsBefore as $ds%}no rpd-ds {$ds.id} {$ds.cable_controller}
            {% onEmpty %}{% endloop %}{% loop in $rpdDsAfter as $ds%}rpd-ds {$ds.id} {$ds.cable_controller} profile {$ds.profile}
            {% onEmpty %}{% endloop %}{% loop in $rpdUsBefore as $us%}no rpd-us {$us.id} {$us.cable_controller}
            {% onEmpty %}{% endloop %}{% loop in $rpdUsAfter as $us%}rpd-us {$us.id} {$us.cable_controller} profile {$us.profile}
            {% onEmpty %}{% endloop %}end""";
    ...
    @Override
    public void writeCurrentAttributes(@NotNull InstanceIdentifier<CoreInterface> instanceIdentifier,
                                       @NotNull CoreInterface coreInterface,
    ...
    @Override
    public void updateCurrentAttributes(@NotNull InstanceIdentifier<CoreInterface> instanceIdentifier,
                                        @NotNull CoreInterface dataBefore,
                                        @NotNull CoreInterface dataAfter,
                                        @NotNull WriteContext writeContext) throws WriteFailedException {
        final String rpdId = instanceIdentifier.firstKeyOf(Rpd.class).getId();
    ...
    @Override
    public void deleteCurrentAttributes(@NotNull InstanceIdentifier<CoreInterface> instanceIdentifier,
                                        @NotNull CoreInterface coreInterface,
                                        @NotNull WriteContext writeContext) throws WriteFailedException {
        final String rpdId = instanceIdentifier.firstKeyOf(Rpd.class).getId();
        Config config = coreInterface.getConfig();
