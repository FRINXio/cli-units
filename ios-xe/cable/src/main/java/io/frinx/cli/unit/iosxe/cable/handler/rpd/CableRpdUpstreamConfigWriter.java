...
public class CableRpdUpstreamConfigWriter implements CliWriter<Config> {

    private static final String WRITE_TEMPLATE = "configure terminal\n"
            + "cable rpd {$rpd}\n"
            + "{% if ($description) %}rpd-us {$upstream} description {$description}\n{% endif %}"
            + "end";

    private static final String UPDATE_TEMPLATE = "configure terminal\n"
            + "cable rpd {$rpd}\n"
            + "no rpd-us {$upstream} description\n"
            + "rpd-us {$upstream} description {$description}\n"
            + "end";
    ...
    public CableRpdUpstreamConfigWriter(Cli cli) {
        this.cli = cli;
    }
    ...
    public void updateCurrentAttributes(@Nonnull InstanceIdentifier<Config> instanceIdentifier,
        ...
        blockingWriteAndRead(cli, instanceIdentifier, dataAfter,
                fT(UPDATE_TEMPLATE, "rpd", rpdId,
                        "upstream", dataAfter.getId(),
                        "description", dataAfter.getDescription()
                ));
    }
