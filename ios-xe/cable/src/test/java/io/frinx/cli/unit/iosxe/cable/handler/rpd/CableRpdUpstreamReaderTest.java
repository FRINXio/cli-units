...
public class CableRpdUpstreamReaderTest {

    private static final String OUTPUT = "cable rpd VFZ-RPD-100\n"
            + " rpd-us 0 description HST-WC0001-DAA001A\n"
            + "cable rpd VFZ-RPD-101\n"
            + " rpd-us 0 description HST-WC0001-DAA002A\n"
            + " rpd-us 1 description HST-WC0001-DAA002B\n"
            + "cable rpd VFZ-RPD-120\n"
            + " rpd-us 0 description HST-WC0001-DAA003A\n"
            + " rpd-us 1 description HST-WC0001-DAA003B\n"
            + "cable rpd VFZ-RPD-121\n";

    @Test
    public void testGetIds() {
        List<UpstreamCommandsKey> keys = CableRpdUpstreamReader.getUpstreamKeys(OUTPUT, "VFZ-RPD-100");
        Assert.assertFalse(keys.isEmpty());
        Assert.assertEquals(Lists.newArrayList("0"),
                keys.stream().map(UpstreamCommandsKey::getId).collect(Collectors.toList()));
    }
}
