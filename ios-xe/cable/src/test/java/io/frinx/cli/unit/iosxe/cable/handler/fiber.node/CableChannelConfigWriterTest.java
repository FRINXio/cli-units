...
public class CableChannelConfigWriterTest {

    private static final String WRITE_INPUT_UPSTREAM = "configure terminal\n"
            + "cable fiber-node 11\n"
            + "upstream Upstream-Cable 1/0/9\n"
            + "end\n";

    private static final String WRITE_INPUT_DOWNSTREAM = "configure terminal\n"
            + "cable fiber-node 11\n"
    ...
    @Test
    public void writeUpTest() throws WriteFailedException {
        writer.writeCurrentAttributes(iidUp, CONFIG_WRITE_UPSTREAM, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        Assert.assertEquals(WRITE_INPUT_UPSTREAM, response.getValue().getContent());
    }

    @Test
    public void writeDownTest() throws WriteFailedException {
        writer.writeCurrentAttributes(iid, CONFIG_WRITE_DOWNSTREAM, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        Assert.assertEquals(WRITE_INPUT_DOWNSTREAM, response.getValue().getContent());
    }
