...
public final class EthernetConfigWriter implements CliWriter<Config> {
    ...
    private static final String WRITE_UPDATE_TEMPLATE = """
            ...
            {% endif %}{% endif %}{% if ($rate) %}lacp rate {$rate}
            {% endif %}{% if ($port_priority) %}lacp port-priority {$port_priority}
            {% else %}{% if ($priority_before) %}no lacp port-priority
            {% endif %}{% endif %}end""";
    ...
    private static final String DELETE_TEMPLATE = """
            ...
            {% endif %}{% if ($channel_before) %}no channel-group
            {% endif %}{% if ($rate) %}no lacp rate
            {% endif %}{% if ($port_priority) %}no lacp port-priority
            {% endif %}end""";
    ...
    public void writeCurrentAttributes(@NotNull InstanceIdentifier<Config> id,
        ...
        blockingWriteAndRead(cli, id, dataAfter,
                fT(WRITE_UPDATE_TEMPLATE,
                        ...
                        "port_priority", getPortPriority(dataAfter, ifcName),
                        "rate", rate));
    }
    ...
    public void updateCurrentAttributes(@NotNull final InstanceIdentifier<Config> id,
        ...
        blockingWriteAndRead(cli, id, dataAfter,
                fT(WRITE_UPDATE_TEMPLATE,
                        ...
                        "channel-group_id", channelGroup,
                        "port_priority", getPortPriority(dataAfter, ifcName),
                        "priority_before", getPortPriority(dataBefore, ifcName),
                        "rate", rate));
    }
    ...
    public void deleteCurrentAttributes(@NotNull InstanceIdentifier<Config> id,
        ...
        blockingDeleteAndRead(cli, id,
                fT(DELETE_TEMPLATE,
                        ...
                        "port_priority", getPortPriority(dataBefore, ifcName)
                ));
    }
    ...
    private Integer getPortPriority(Config dataAfter, String ifcName) {
        final var aug = dataAfter.getAugmentation(IfEthCiscoExtAug.class);
        if (aug == null || aug.getLacpPortPriority() == null) {
            return null;
        }
        return aug.getLacpPortPriority();
    }
    ...
    private String getLacpRate(Config datatAfter) {
        ...
        return aug.getLacpRate().getName().toLowerCase(Locale.ROOT);
    }
}