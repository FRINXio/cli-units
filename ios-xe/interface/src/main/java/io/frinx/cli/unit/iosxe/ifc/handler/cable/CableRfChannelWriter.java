...
public class CableRfChannelWriter implements CliWriter<RfChannels> {
    ...
    private static final String WRITE_TEMPLATE = "configure terminal\n"
            + "interface {$ifc_name}\n"
            + "cable rf-channels channel-list {$config.channel_list}"
            + " bandwidth-percent {$config.bandwidth_percent}\n"
            + "end";
    ...
    @Override
    public void writeCurrentAttributes(@Nonnull InstanceIdentifier<RfChannels> instanceIdentifier,
                                       @Nonnull RfChannels config,
    ...
    @Override
    public void updateCurrentAttributes(@Nonnull InstanceIdentifier<RfChannels> instanceIdentifier,
                                        @Nonnull RfChannels dataBefore,
                                        @Nonnull RfChannels dataAfter,
                                        @Nonnull WriteContext writeContext) throws WriteFailedException {
        final String ifcName = instanceIdentifier.firstKeyOf(Interface.class).getName();
    ...
    @Override
    public void deleteCurrentAttributes(@Nonnull InstanceIdentifier<RfChannels> instanceIdentifier,
                                        @Nonnull RfChannels config,
                                        @Nonnull WriteContext writeContext) throws WriteFailedException {
        final String ifcName = instanceIdentifier.firstKeyOf(Interface.class).getName();
        blockingWriteAndRead(cli, instanceIdentifier, config,
