...
public class CfmMipWriter implements CliWriter<Level> {
    private static String WRITE_TEMPLATE = "configure terminal\n"
            ...
            + "{% if ($vlanAdd) %}ethernet cfm mip level {$level} vlan {$vlanAdd}\n{% endif %}"
            + "end";

    private static String DELETE_TEMPLATE = "configure terminal\n"
            + "interface {$ifc_name}\n"
    ...
    @Override
    public void writeCurrentAttributes(@Nonnull InstanceIdentifier<Level> id,
                                       @Nonnull Level level,
    ...
    @Override
    public void updateCurrentAttributes(@Nonnull InstanceIdentifier<Level> id,
                                        @Nonnull Level dataBefore,
                                        @Nonnull Level dataAfter,
    ...
    @Override
    public void deleteCurrentAttributes(@Nonnull InstanceIdentifier<Level> id,
                                        @Nonnull Level level,
                                        @Nonnull WriteContext ctx) throws WriteFailedException {
        final Short levelValue = id.firstKeyOf(Level.class).getLevel();
        String ifcName = id.firstKeyOf(Interface.class).getName();
