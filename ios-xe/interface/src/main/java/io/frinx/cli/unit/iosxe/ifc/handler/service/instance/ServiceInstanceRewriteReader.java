...
public class ServiceInstanceRewriteReader implements CliConfigReader<Rewrite, RewriteBuilder> {
    ...
    @Override
    public void readCurrentAttributes(@NotNull InstanceIdentifier<Rewrite> instanceIdentifier,
                                      @NotNull RewriteBuilder rewriteBuilder,
    ...
    private static void setOperation(String output, RewriteBuilder rewriteBuilder) {
        ...
        if (operation.isPresent()) {
            switch (operation.get()) {
                case "pop" -> rewriteBuilder.setOperation(Operation.Pop);
                case "push" -> rewriteBuilder.setOperation(Operation.Push);
                case "translate" -> rewriteBuilder.setOperation(Operation.Translate);
                default -> throw new IllegalArgumentException("Cannot parse Operation value: " + operation.get());
            }
        }
    }
    ...
    private static void setType(String output, RewriteBuilder rewriteBuilder) {
        ...
        if (type.isPresent()) {
            switch (type.get()) {
                case "ingress" -> rewriteBuilder.setType(Type.Ingress);
                case "egress" -> rewriteBuilder.setType(Type.Egress);
                default -> throw new IllegalArgumentException("Cannot parse Type value: " + type.get());
            }
        }
    }
}