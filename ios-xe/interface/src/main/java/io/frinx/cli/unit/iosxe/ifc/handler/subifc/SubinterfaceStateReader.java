...
public final class SubinterfaceStateReader implements CliOperReader<State, StateBuilder> {
    ...
    @Override
    public void readCurrentAttributes(@Nonnull InstanceIdentifier<State> id,
        ...
        final String output = blockingRead(f(InterfaceStateReader.SH_SINGLE_INTERFACE, subIfcName), cli, id, ctx);
        parseInterfaceState(output, builder, subKey.getIndex(), subIfcName);
    }

    public static void parseInterfaceState(final String output,
        ...
        ParsingUtils.parseField(output,
            InterfaceStateReader.DESCRIPTION_LINE::matcher,
