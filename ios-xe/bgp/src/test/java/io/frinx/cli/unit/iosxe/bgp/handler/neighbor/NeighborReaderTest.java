...
public class NeighborReaderTest {
    ...
    private String summOutput = "Neighbor        V           AS MsgRcvd MsgSent   TblVer  InQ OutQ Up/Down  "
            + "State/PfxRcd\n"
    ...
    public void testNeighborIds() {
        ...
        Assert.assertEquals("IDLE", stateBuilder.getSessionState().getName());

        PrefixesBuilder prefixesBuilder = new PrefixesBuilder();
        PrefixesReader.parsePrefixes(summOutput, prefixesBuilder, "99.0.0.2");
        Assert.assertEquals(Long.valueOf(2), prefixesBuilder.getReceived());
    }
}
