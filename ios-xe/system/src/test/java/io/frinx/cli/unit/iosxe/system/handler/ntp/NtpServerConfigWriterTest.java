...
public class NtpServerConfigWriterTest {

    private static final String WRITE_INPUT = "configure terminal\n"
            + "ntp server 198.18.1.16\n"
            + "end\n";

    private static final String WRITE_INPUT_VRF = "configure terminal\n"
            + "ntp server vrf MANAGEMENT 198.18.1.16\n"
            + "end\n";

    private static final String UPDATE_INPUT = "configure terminal\n"
            + "no ntp server 198.18.1.16\n"
            + "end\n";

    private static final String DELETE_INPUT = "configure terminal\n"
            + "no ntp server 198.18.1.15\n"
            + "end\n";

    private static final Config CONFIG_WRITE = new ConfigBuilder()
            .setAddress(new Host(new IpAddress(new Ipv4Address("198.18.1.16"))))
            .build();

    private static final Config CONFIG_WRITE_VRF = new ConfigBuilder()
            .setAddress(new Host(new IpAddress(new Ipv4Address("198.18.1.16"))))
            .addAugmentation(VrfCiscoAug.class, new VrfCiscoAugBuilder()
                    .setVrf("MANAGEMENT")
                .build())
            .build();
    ...
    private final InstanceIdentifier iid = IIDs.SY_NT_SERVERS
            .child(Server.class, new ServerKey(new Host(new IpAddress(new Ipv4Address("198.18.1.16")))))
            .child(Config.class);
    ...
    @Test
    public void writeVrfTest() throws WriteFailedException {
        writer.writeCurrentAttributes(iid, CONFIG_WRITE_VRF, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        Assert.assertEquals(WRITE_INPUT_VRF, response.getValue().getContent());
    }

    @Test
    public void updateTest() throws WriteFailedException {
        writer.updateCurrentAttributes(iid, CONFIG_WRITE, CONFIG_UPDATED, context);
