...
public class ConfigMetadataReader implements CliOperReader<ConfigurationMetadata, ConfigurationMetadataBuilder> {
    ...
    private static final String SHOW_LAST_COMMIT_TIME = "show running-config | include Last configuration";
    private static final Pattern PATTERN =
            Pattern.compile("! Last configuration change at "
                    + "(?<time>\\d{2}:\\d{2}:\\d{2} [A-Z]+ [A-Za-z]+ [A-Za-z]+ \\d{1,2} \\d{4})?(?<user>.+)*");
    ...
    @VisibleForTesting
    public static Optional<String> getLastConfigurationFingerprint(final String timeFormat) {
        final Optional<String> output = ParsingUtils.parseField(timeFormat, 0,
            PATTERN::matcher,
            m -> m.group("time"));
        return output.isEmpty() ? Optional.empty() : output;
    }
    ...
    public void readCurrentAttributes(@NotNull InstanceIdentifier<ConfigurationMetadata> instanceIdentifier,
        ...
        final String output = blockingRead(SHOW_LAST_COMMIT_TIME, cli, instanceIdentifier, readContext);
        final Optional<String> data = getLastConfigurationFingerprint(output);
