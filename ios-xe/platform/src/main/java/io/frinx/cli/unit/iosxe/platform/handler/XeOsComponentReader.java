...
public class XeOsComponentReader implements CliOperListReader<Component, ComponentKey, ComponentBuilder> {

    private Cli cli;

    public XeOsComponentReader(Cli cli) {
        this.cli = cli;
    }

    static final String SH_PLATFORM = "show platform";
    static final Pattern SLOT_LINE = Pattern.compile(
            "(?<slot>\\S+) *(?<type>\\S+) *(?<state>\\S+,* *\\S*) *(?<insertTime>\\S+) *");

    private static final String SH_MODULE = "show inventory";
    static final Pattern LINE = Pattern.compile("NAME: \"(?<name>[^\"]+)\".*");
    ...
    public List<ComponentKey> getAllIds(@NotNull InstanceIdentifier<Component> id,
        ...
        var componentKeys = parseAllComponentIds(blockingRead(SH_MODULE, cli, id, context));
        componentKeys.addAll(parseAllSlotIds(blockingRead(SH_PLATFORM, cli, id, context)));
        componentKeys.addAll(Collections.singletonList(OsComponent.OS_KEY));
        return componentKeys;
    }

    static List<ComponentKey> parseAllComponentIds(String output) {
    ...
    static List<ComponentKey> parseAllSlotIds(String output) {
        return ParsingUtils.parseFields(output, 0,
                SLOT_LINE::matcher,
                matcher -> matcher.group("slot"),
                ComponentKey::new);
    }
}