...
package io.frinx.cli.unit.iosxe.network.instance.handler.policy.forwarding;
...
class PolicyForwardingInterfaceConfigWriterTest {
    ...
    private static final String UPDATE_INPUT = """
            interface Loopback0
            no service-policy input input-pol
            service-policy input input-pol1
            no service-policy output output-pol
            root
            """;
    ...
    @Test
    void writeSubif() throws WriteFailedException {
        this.writer.writeCurrentAttributes(iidSubif, data, context);

        Mockito.verify(cli)
                .executeAndRead(response.capture());
        assertEquals(WRITE_INPUT_SUBIF, response.getValue()
                .getContent());
    }
    ...
    @Test
    void deleteSubif() throws WriteFailedException {
        this.writer.deleteCurrentAttributes(iidSubif, data, context);

        Mockito.verify(cli)
                .executeAndRead(response.capture());
        assertEquals(DELETE_INPUT_SUBIF, response.getValue()
                .getContent());
    }
}
