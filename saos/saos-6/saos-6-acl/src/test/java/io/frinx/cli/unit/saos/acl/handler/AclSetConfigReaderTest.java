...
class AclSetConfigReaderTest {
    ...
    private void buildAndTest(String aclProfile, Class<? extends FORWARDINGACTION>  expDefaultFwdAction,
        ...
        assertEquals(aclProfile, builder.getName());
        assertEquals(ACLIPV4.class, builder.getType());
        assertEquals(expDefaultFwdAction, builder.getAugmentation(Saos6AclSetAug.class).getDefaultFwdAction());
        assertEquals(expEnabled ,builder.getAugmentation(Saos6AclSetAug.class).isEnabled());
    }
}
