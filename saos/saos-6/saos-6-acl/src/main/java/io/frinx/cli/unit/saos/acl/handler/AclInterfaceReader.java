...
public class AclInterfaceReader implements CliConfigListReader<Interface, InterfaceKey, InterfaceBuilder> {
    ...
    private static final String SH_INTERFACES = "configuration search string ingress-acl";
    private static final Pattern PORT_LINE = Pattern.compile("port set port (?<name>\\S+).*");
    ...
    public List<InterfaceKey> getAllIds(@NotNull InstanceIdentifier<Interface> instanceIdentifier,
        ...
        return getInterfaceKeys(blockingRead(SH_INTERFACES, cli, instanceIdentifier, readContext));
    }

    @VisibleForTesting
    public static List<InterfaceKey> getInterfaceKeys(final String output) {
        return ParsingUtils.parseFields(output, 0,
            PORT_LINE::matcher,
            matcher -> matcher.group("name"),
            id -> new InterfaceKey(new InterfaceId(id)));
    }
