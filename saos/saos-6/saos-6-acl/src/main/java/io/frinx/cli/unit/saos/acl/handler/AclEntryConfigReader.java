...
import org.opendaylight.yang.gen.v1.http.frinx.openconfig.net.yang.acl.rev170526.access.list.entries.top.acl.entries.acl.entry.Config;
...
public class AclEntryConfigReader implements CliConfigReader<Config, ConfigBuilder> {

    private final Cli cli;

    public AclEntryConfigReader(Cli cli) {
        this.cli = cli;
    }
    ...
    void parseConfig(String output, ConfigBuilder builder, String aclSetName, Long sequenceId) {
        Config2Builder augBuilder = new Config2Builder();

        builder.setSequenceId(sequenceId);
        setRuleName(output, augBuilder, aclSetName, sequenceId);

        builder.addAugmentation(Config2.class, augBuilder.build());
    }

    private void setRuleName(String output, Config2Builder builder, String aclSetName, Long sequenceId) {
        Pattern pattern = Pattern.compile(".*" + aclSetName + " rule (?<name>\\S+) precedence " + sequenceId + ".*");
