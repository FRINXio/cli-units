...
public class RelayAgentConfigReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    static void parseRelayAgentConfig(String output, ConfigBuilder builder) {
        ...
        setReplaceOption82(output, builder);
    }
    ...
    private static void setReplaceOption82(String output, ConfigBuilder builder) {
        Pattern pattern = Pattern.compile("dhcp l2-relay-agent .* replace-option82 (?<replaceOption>\\S+).*");

        builder.setReplaceOption82(false);
        ParsingUtils.parseField(output, 0,
            pattern::matcher,
            matcher -> matcher.group("replaceOption"),
            replaceOption -> builder.setReplaceOption82(Objects.equals(replaceOption, "on")));
    }
}