...
class RelayAgentConfigReaderTest {
    ...
    private static final String OUTPUT = "dhcp l2-relay-agent enable\n"
            + "dhcp l2-relay-agent set remote-id-type device-hostname replace-option82 off";

    private static final Config CONFIG = new ConfigBuilder()
            .setEnable(true)
            .setRemoteIdType(Config.RemoteIdType.DeviceHostname)
            .setReplaceOption82(false)
            .build();
