...
public class NetworkInstanceConfigWriterTest {
    ...
    @Test
    public void writeNetworkInstance() throws Exception {
        ...
        Assert.assertEquals("L2VSICP", ((Config) sortEntries.get(0).getValue()).getType().getSimpleName());
    }

    @Test
    public void updateNetworkInstance() throws Exception {
        final NetworkInstanceConfigWriter networkInstanceConfigWriter = new NetworkInstanceConfigWriter(cli);
        WriteContext mock = Mockito.mock(WriteContext.class);
        ModificationCache cache = new ModificationCache();
    ...
    @Test
    public void deleteNetworkInstance() throws Exception {
        final NetworkInstanceConfigWriter networkInstanceConfigWriter = new NetworkInstanceConfigWriter(cli);
        WriteContext mock = Mockito.mock(WriteContext.class);
        ModificationCache cache = new ModificationCache();
        cache.put(IIDs.NE_NE_CONFIG + "_updatesCount", 3);
        ...
        Assert.assertEquals("L2VSI", ((Config) sortEntries.get(0).getValue()).getType().getSimpleName());
    }
}