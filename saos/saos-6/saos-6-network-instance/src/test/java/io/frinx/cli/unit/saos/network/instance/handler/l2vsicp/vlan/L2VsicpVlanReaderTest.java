...
class L2VsicpVlanReaderTest {
    ...
    @Test
    void parseVlanKeyTest() {
        assertEquals(L2vsicpVlanReader.getVlanKeys(OUTPUT, "vc2").get(0), new VlanKey(new VlanId(3)));
        assertEquals(L2vsicpVlanReader.getVlanKeys(OUTPUT, "Name").get(0), new VlanKey(new VlanId(7)));
        assertEquals(L2vsicpVlanReader.getVlanKeys(OUTPUT, "notExists"), Collections.emptyList());
    }
}