...
public class L2VSISubPortConfigReaderTest {

    @Test
    public void parseSubPortConfigTest() {
        L2VSISubPortConfigReader reader = new L2VSISubPortConfigReader(Mockito.mock(Cli.class));
        ConfigBuilder builder = new ConfigBuilder();

        L2VSISubPortConfigReader.parseSubPortConfig(builder, "LAG=LM01E_IPTV_800_1");

        Assert.assertEquals("LAG=LM01E_IPTV_800_1", builder.getId());
        Assert.assertEquals("LAG=LM01E_IPTV_800_1", builder.getInterface());
        Assert.assertEquals("Ieee8023adLag", builder.getAugmentation(Saos8NiIfcAug.class)
