...
public class L2VSICpuSubinterfaceConfigReader implements CliConfigReader<Config, ConfigBuilder>,
        CompositeReader.Child<Config, ConfigBuilder> {

    private Cli cli;

    public L2VSICpuSubinterfaceConfigReader(Cli cli) {
        this.cli = cli;
    }
    ...
    public void readCurrentAttributes(@Nonnull InstanceIdentifier<Config> instanceIdentifier,
        ...
        if (isL2VLAN(instanceIdentifier, readContext, vsName)) {
            parseSubInterfaceConfig(configBuilder, instanceIdentifier.firstKeyOf(Interface.class).getId());
        }
    }

    @VisibleForTesting
    static void parseSubInterfaceConfig(ConfigBuilder configBuilder, String id) {

        Saos8NiIfcAugBuilder augBuilder = new Saos8NiIfcAugBuilder();
        configBuilder.setId(id);
        augBuilder.setType(L2vlan.class);

        configBuilder.addAugmentation(Saos8NiIfcAug.class, augBuilder.build());
    }

    private boolean isL2VLAN(InstanceIdentifier<Config> id, ReadContext readContext,
                             String vsName) throws ReadFailedException {
        return L2VSICpuSubinterfaceReader.getAllIds(cli, this, id, readContext, vsName)
