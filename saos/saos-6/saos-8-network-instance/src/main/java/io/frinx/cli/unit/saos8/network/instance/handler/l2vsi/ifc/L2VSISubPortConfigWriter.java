...
public class L2VSISubPortConfigWriter implements CompositeWriter.Child<Config>, CliWriter<Config> {
    ...
    public L2VSISubPortConfigWriter(Cli cli) {
        this.cli = cli;
    }
    ...
    public boolean writeCurrentAttributesWResult(@Nonnull InstanceIdentifier<Config> iid,
        ...
        if (!data.getAugmentation(Saos8NiIfcAug.class).getType().equals(Ieee8023adLag.class)) {
            return false;
        }
        blockingWriteAndRead(fT(WRITE_SUBPORT, "data", data,
    ...
    public boolean updateCurrentAttributesWResult(@Nonnull InstanceIdentifier<Config> iid,
        ...
        throw new WriteFailedException.UpdateFailedException(iid, dataBefore, dataAfter,
                new IllegalArgumentException("Sub-port interface cannot be updated"));
    }
    ...
    public boolean deleteCurrentAttributesWResult(@Nonnull InstanceIdentifier<Config> iid,
                                                  @Nonnull Config dataBefore,
                                                  @Nonnull WriteContext writeContext) throws WriteFailedException {
        if (!dataBefore.getAugmentation(Saos8NiIfcAug.class).getType().equals(Ieee8023adLag.class)) {
            return false;
        }
        blockingDeleteAndRead(fT(DELETE_SUBPORT, "data", dataBefore), cli, iid);
        return true;
    }
}