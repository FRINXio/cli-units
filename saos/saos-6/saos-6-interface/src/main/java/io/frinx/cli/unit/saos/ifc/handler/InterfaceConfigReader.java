...
public class InterfaceConfigReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    private void setSpeedType(final String output, final IfSaosAugBuilder ifSaosAugBuilder, String name) {
        ...
        if (speedTypeValue.isPresent()) {
            SaosIfExtensionConfig.SpeedType speed = switch (speedTypeValue.get()) {
                case "auto" -> SaosIfExtensionConfig.SpeedType.Auto;
                case "ten" -> SaosIfExtensionConfig.SpeedType.Ten;
                case "hundred" -> SaosIfExtensionConfig.SpeedType.Hundred;
                case "gigabit" -> SaosIfExtensionConfig.SpeedType.Gigabit;
                case "ten-gig" -> SaosIfExtensionConfig.SpeedType.TenGig;
                default -> throw new IllegalArgumentException("Cannot parse speed-type value: " + speedTypeValue.get());
            };
