...
public class BroadcastContainmentFilterConfigWriter implements CliWriter<Config> {
    ...
    private static final String WRITE_TEMPLATE = """
            ...
            {% endif %}{% if ($containList) %}broadcast-containment set filter {$data.name} containment-classification {$containList}
            {% endif %}""";

    @SuppressWarnings("checkstyle:linelength")
    private static final String UPDATE_TEMPLATE = """
            {% if ($rateCompare) %}broadcast-containment set filter {$data.name} kbps {$data.rate}
            {% endif %}{% if ($classificationCompare) %}broadcast-containment set filter {$data.name} containment-classification {$containList}
            {% endif %}""";
