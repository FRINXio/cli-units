...
public class P2pAttributesConfigWriter implements CliWriter<Config> {
    ...
    private static final String WRITE_TEMPLATE = """
            configure terminal
            interface tunnel {$name}{% if ($delete) %}no {% endif %}tunnel destination {$destination}
            end
            """;
    ...
    public void writeCurrentAttributes(@NotNull InstanceIdentifier<Config> id, @NotNull Config data, @NotNull
        ...
        final String tunnelInterfaceName = id.firstKeyOf(Interface.class).getInterfaceId().getValue();
    ...
    private String getCommand(String tunnelInterfaceName, Config data, boolean delete) {
        return fT(WRITE_TEMPLATE,"name",tunnelInterfaceName,"destination", data.getDestination()
                .getIpv4Address().getValue(),"delete", delete ? Chunk.TRUE : null);
