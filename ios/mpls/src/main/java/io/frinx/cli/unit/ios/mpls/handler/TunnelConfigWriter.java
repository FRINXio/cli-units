...
public class TunnelConfigWriter implements CliWriter<Config> {
    ...
    private static final String INPUT_T = """
            {% if ($delete) %}no {% endif %}interface tunnel {$name}
            {% if (!$delete) %}{% if (!$autoroute) %}no tunnel mpls traffic-eng autoroute announce
            {% else if ($autoroute) %}tunnel mode mpls traffic-eng
            tunnel mpls traffic-eng autoroute announce
            {% endif %}{% if ($autoroute) %}{% if ($metric == nondefined) %}no tunnel mpls traffic-eng autoroute metric
            {% else if ( $metric != $null) %}tunnel mpls traffic-eng autoroute metric absolute {$metric}
            {% endif %}{% endif %}end{% endif %}""";
