...
public class BgpActionsConfigReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    private static BgpOriginAttrType parseOrigin(final String value) {
        return switch (value) {
            case "egp" -> BgpOriginAttrType.EGP;
            case "igp" -> BgpOriginAttrType.IGP;
            case "incomplete" -> BgpOriginAttrType.INCOMPLETE;
            default -> throw new IllegalArgumentException("Did not match origin for value: " + value);
        };
    }
}