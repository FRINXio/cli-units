...
public class SetCommunityInlineConfigReader implements CliConfigReader<Config, ConfigBuilder> {

    private final Cli cli;

    public SetCommunityInlineConfigReader(Cli cli) {
        this.cli = cli;
    }
    ...
    static void parseConfig(final String routeMapName,
        ...
        ParsingUtils.parseField(routeMapOutput, 0,
            SetCommunityConfigReader.COMMUNITY_LINE::matcher,
            matcher -> matcher.group("values"),
            s -> configBuilder.setCommunities(parseCommunities(s)));
    }

    private static List<SetCommunityInlineConfig.Communities> parseCommunities(final String output) {
