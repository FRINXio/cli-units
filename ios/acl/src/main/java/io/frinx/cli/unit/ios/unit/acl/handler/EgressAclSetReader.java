...
public class EgressAclSetReader implements CliConfigListReader<EgressAclSet, EgressAclSetKey, EgressAclSetBuilder> {
    ...
    private static final Pattern IPV4_EGRESS_ACLS_LINE =
            Pattern.compile("\\s*ip access-group (?<name>.+) out.*", Pattern.DOTALL);
    private static final Pattern IPV6_EGRESS_ACLS_LINE =
            Pattern.compile("\\s*ipv6 traffic-filter (?<name>.+) out.*", Pattern.DOTALL);
    ...
    private static Pattern aclV4PatternByName(final String name) {
        final String regex = String.format("\\s*ip access-group %s out", name);
        return Pattern.compile(regex);
    }

    private static Pattern aclV6PatternByName(final String name) {
        final String regex = String.format("\\s*ipv6 traffic-filter %s out", name);
        return Pattern.compile(regex);
    }
    ...
    public static List<EgressAclSetKey> parseAclKeys(String output) {
        List<EgressAclSetKey> keys = new ArrayList<>();
        keys.addAll(ParsingUtils.parseFields(output, 0,
            IPV4_EGRESS_ACLS_LINE::matcher,
            matcher -> matcher.group("name"),
            name -> new EgressAclSetKey(name, ACLIPV4.class)
        ));
        keys.addAll(ParsingUtils.parseFields(output, 0,
            IPV6_EGRESS_ACLS_LINE::matcher,
            matcher -> matcher.group("name"),
            name -> new EgressAclSetKey(name, ACLIPV6.class)
        ));
        return keys;
    }
    ...
    @NotNull
    @Override
    public List<EgressAclSetKey> getAllIds(@NotNull InstanceIdentifier<EgressAclSet> instanceIdentifier,
    ...
    @Override
    public void readCurrentAttributes(@NotNull InstanceIdentifier<EgressAclSet> instanceIdentifier,
                                      @NotNull EgressAclSetBuilder egressAclSetBuilder,
                                      @NotNull ReadContext readContext) throws ReadFailedException {
        ...
        egressAclSetBuilder.setSetName(entry.getName());
        egressAclSetBuilder.setType(entry.getType());
    }
}