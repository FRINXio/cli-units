...
final class AclEntryLineParser {
    ...
    static Optional<String> findIpv6LineWithSequenceId(long sequenceId, String lines) {
        Pattern pattern = Pattern.compile("^\\s?sequence " + sequenceId + ".*$", Pattern.MULTILINE);
        Matcher matcher = pattern.matcher(lines);
        if (matcher.find()) {
    ...
    static void parseLine(final AclEntryBuilder builder, String line, Class<? extends ACLTYPE> aclType) {
        ...
        else { // else it's ipv6 acl
            // parse sequence ID if it's present
            String nextWord = words.peek();
            if ("sequence".equals(nextWord)) {
                words.poll();
                parseSequenceId(builder, words.poll());
            }
        }
    ...
    private static boolean isMultiple(Queue<String> words, String operation) {
        return operation != null && (operation.equals("eq") || operation.equals("neq"))
                ...
                && ((StringUtils.isNumeric(words.peek()) && Integer.parseInt(words.peek()) >= 0
                && Integer.parseInt(words.peek()) <= MAX_PORT_NUMBER)
    ...
    private static Optional<AclPrecedenceAug> getPrecedence(String precedence) {
        if (precedence != null) {
            ...
            return Optional.of(new AclPrecedenceAugBuilder()
                    .setPrecedence(Precedence.valueOf(precedence.toUpperCase(Locale.ROOT))).build());
        }
        return Optional.empty();
    }
    ...
    private static Optional<AclOptionAug> getOptions(String option) {
        if (option != null && StringUtils.isNumeric(option)) {
            ...
            return Optional.of(new AclOptionAugBuilder()
                    .setOption(new Option(Option.Enumeration.valueOf(option.toUpperCase(Locale.ROOT)))).build());
        }
        return Optional.empty();
    }
