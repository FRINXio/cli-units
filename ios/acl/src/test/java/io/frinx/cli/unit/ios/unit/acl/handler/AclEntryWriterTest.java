...
class AclEntryWriterTest {
    ...
    private static final String ACL_EXTENDED_WRITE_EQ = """
            configure terminal
            ip access-list extended TEST
            10 deny udp 10.10.99.0 0.0.0.254 eq 545 68 0.0.0.0 255.255.255.255    \s
            end
            """;

    private static final String ACL_EXTENDED_WRITE_NEQ = """
            configure terminal
            ip access-list extended TEST
            10 deny udp 10.10.99.0 0.0.0.254 neq 68 545 0.0.0.0 255.255.255.255    \s
            end
            """;

    private static final String ACL_EXTENDED_WRITE_PRECEDENCE = """
            configure terminal
            ip access-list extended TEST
            10 deny tcp 0.0.0.0 255.255.255.255  0.0.0.0 255.255.255.255   precedence internet \s
            end
            """;

    private static final String ACL_EXTENDED_WRITE_OPTION = """
            configure terminal
            ip access-list extended TEST
            10 deny ip 0.0.0.0 255.255.255.255 0.0.0.0 255.255.255.255  option record-route\s
            end
            """;

    private static final String ACL_EXTENDED_WRITE_ESTABLISHED_TRUE = """
            configure terminal
            ip access-list extended TEST
            10 permit tcp 1.1.1.1 0.0.0.0  2.2.2.2 0.0.0.0  established  \s
            end
            """;
    ...
    @Test
    void testExtendedWriteEq() throws WriteFailedException {
        ...
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(ACL_EXTENDED_WRITE_EQ, response.getValue().getContent());
    }

    @Test
    void testExtendedWriteNeq() throws WriteFailedException {
        final AclEntry aclEntry = getExtendedAclEntry(10L, "deny", "udp",
        ...
        writer.writeCurrentAttributes(piidext, aclEntry, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(ACL_EXTENDED_WRITE_NEQ, response.getValue().getContent());
    }

    @Test
    void testExtendedWritePrecedence() throws WriteFailedException {
        ...
        writer.writeCurrentAttributes(piidext, aclEntry, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(ACL_EXTENDED_WRITE_PRECEDENCE, response.getValue().getContent());
    }

    @Test
    void testExtendedWriteOption() throws WriteFailedException {
        ...
        writer.writeCurrentAttributes(piidext, aclEntry, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(ACL_EXTENDED_WRITE_OPTION, response.getValue().getContent());
    }

    @Test
    void testExtendedWriteEstablished() throws WriteFailedException {
        final AclEntry aclEntry = getExtendedAclEntry(10L, "permit", "tcp",
                "1.1.1.1", "0.0.0.0", "2.2.2.2", "0.0.0.0", null, null,
                null, null, null, null, null, true);
        writer.writeCurrentAttributes(piidext, aclEntry, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(ACL_EXTENDED_WRITE_ESTABLISHED_TRUE, response.getValue().getContent());
    }
    ...
    @Test
    void processIpv6_srcDst_addresses() {
        ...
        assertEquals("FE80:0000:0000:0000:0202:B3FF:FE1E:1111/96", result.aclDstAddr);
        assertEquals("FE80:0000:0000:0000:0202:B3FF:FE1E:2222/96", result.aclSrcAddr);
        ...
        AclEntryWriter.processIpv6(aclEntry, result);
        assertEquals("0:0:0:0:0:ffff:192.0.2.1/96", result.aclDstAddr);
        assertEquals("0:0:0:0:0:ffff:192.0.2.1/96", result.aclSrcAddr);
        ...
        result = new AclEntryWriter.MaxMetricCommandDTO();
        AclEntryWriter.processIpv6(aclEntry, result);
        assertEquals("::ffff:192.0.2.1/96", result.aclDstAddr);
        assertEquals("::ffff:192.0.2.1/96", result.aclSrcAddr);
    }
}