...
public class PrivilegeWriter implements CliWriter<Privilege> {
    ...
    private static final String WRITE_TEMPLATE = "configure terminal\n"
            + "{% if ($levels.level) %}"
            + "{% loop in $levels.level as $level %}"
            + "{% if ($level.config.id) %}"
            + "{% if ($level.config.commands) %}"
            + "{% loop in $level.config.commands as $command %}"
            + "privilege exec level {$level.config.id.value} {$command}\n"
            + "{% endloop %}"
            + "{% endif %}"
            + "{% endif %}"
            + "{% endloop %}"
    ...
    @Override
    public void writeCurrentAttributes(@Nonnull InstanceIdentifier<Privilege> instanceIdentifier,
                                       @Nonnull Privilege privilege,
    ...
    @Override
    public void updateCurrentAttributes(@Nonnull InstanceIdentifier<Privilege> id,
                                        @Nonnull Privilege dataBefore,
                                        @Nonnull Privilege dataAfter,
                                        @Nonnull WriteContext writeContext) throws WriteFailedException {
    ...
    @Override
    public void deleteCurrentAttributes(@Nonnull InstanceIdentifier<Privilege> instanceIdentifier,
                                        @Nonnull Privilege privilege,
                                        @Nonnull WriteContext writeContext) throws WriteFailedException {
        blockingWriteAndRead(cli, instanceIdentifier, privilege,
