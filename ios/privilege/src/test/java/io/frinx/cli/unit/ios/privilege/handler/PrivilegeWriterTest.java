...
public class PrivilegeWriterTest {
    ...
    public void isConfigValid() {
        ...
        Assert.assertTrue(PrivilegeWriter.getCommandsWithoutParent(invalidConfig).size() > 0);
        Assert.assertFalse(PrivilegeWriter.getCommandsWithoutParent(LEVELS).size() > 0);
    }
