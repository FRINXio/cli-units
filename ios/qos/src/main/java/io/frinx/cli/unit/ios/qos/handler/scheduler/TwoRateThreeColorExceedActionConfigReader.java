...
import org.opendaylight.yang.gen.v1.http.frinx.openconfig.net.yang.qos.rev161216.qos.scheduler._2r3c.top.two.rate.three.color.exceed.action.Config;
...
public class TwoRateThreeColorExceedActionConfigReader implements CliConfigReader<Config, ConfigBuilder> {

    private static final Pattern EXCEED_ACTION = Pattern.compile("exceed-action (?<action>.+)");

    private Cli cli;

    public TwoRateThreeColorExceedActionConfigReader(Cli cli) {
        this.cli = cli;
    }
