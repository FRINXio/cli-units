...
public class InputReader implements CliConfigListReader<Input, InputKey, InputBuilder> {
    ...
    @NotNull
    @Override
    public List<InputKey> getAllIds(@NotNull InstanceIdentifier<Input> instanceIdentifier,
        ...
        final String output = blockingRead(f(SchedulerReader.SH_POLICY_MAP_CLASSES, policyName),
    ...
    public static List<InputKey> getInputKeys(final String output, final Long seq) {
        ...
        final List<InputKey> keys = new ArrayList<>(1);
        ParsingUtils.parseField(output, seq.intValue() - 1,
            SchedulerReader.CLASS_NAME_LINE::matcher,
            m -> m.group("name"),
            k -> keys.add(new InputKey(k)));
        return keys;
    }

    @Override
    public void readCurrentAttributes(@NotNull InstanceIdentifier<Input> instanceIdentifier,
                                      @NotNull InputBuilder inputBuilder,
