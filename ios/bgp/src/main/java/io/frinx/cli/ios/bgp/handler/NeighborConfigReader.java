...
public class NeighborConfigReader implements BgpReader.BgpConfigReader<Config, ConfigBuilder> {
    ...
    private static final String SH_SUMM = "sh run | include ^router bgp|^ *address-family|^ *neighbor %s";
    ...
    @Override public void readCurrentAttributesForType(@Nonnull InstanceIdentifier<Config> instanceIdentifier,
                                                       @Nonnull ConfigBuilder configBuilder,
                                                       @Nonnull ReadContext readContext) throws ReadFailedException {
        String vrfName = instanceIdentifier.firstKeyOf(NetworkInstance.class).getName();
        String ipAddress =
            instanceIdentifier.firstKeyOf(Neighbor.class).getNeighborAddress().getIpv4Address().getValue();
    ...
    private void parseConfigAttributes(String output, ConfigBuilder configBuilder, String vrfName) {
        ...
        if(DEFAULT_BGP_INSTANCE.equals(vrfName)) {
            if (vrfSplit.length > 1 && vrfSplit[0] != null) {
                parseDefault(configBuilder, vrfSplit[0]);
            }
        } else {
    ...
    private void parseDefault(ConfigBuilder configBuilder, String output) {
        setAttributes(configBuilder, output);
    }
    ...
    private void setAttributes(ConfigBuilder configBuilder, String defaultInstance) {
