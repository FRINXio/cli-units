...
package io.frinx.cli.unit.ios.bgp.handler.table;
...
public class BgpTableConnectionWriter implements CompositeWriter.Child<Config>, CliWriter<Config> {
    ...
    private static final String UPDATE_REDIS = """
            configure terminal
            router bgp {$bgp}
            address-family {$family}{.if ($vrf) } vrf {$vrf}{/if}
            {.if ($add) }{.else}no {/if}redistribute {$protocol} {$protocol_id}{.if ($vrf) } vrf {$vrf}{/if}{.if ($add) }{.if ($policy) } route-map {$policy}{/if}{/if}
            end""";
    ...
    public boolean writeCurrentAttributesWResult(@NotNull InstanceIdentifier<Config> instanceIdentifier,
        ...
        if (config.getDstProtocol().equals(BGP.class)) {
            List<Protocol> allProtocols = writeContext.readAfter(RWUtils.cutId(instanceIdentifier, IIDs
                    ...
                    .orElse(new ProtocolsBuilder().setProtocol(Collections.emptyList()).build())
                    .getProtocol();
