...
public class InterfaceVlanReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    public static List<VlanSwitchedConfig.TrunkVlans> getSwitchportTrunkAllowedVlanList(String allowedValues) {
        ...
        return splitMultipleVlan(vlanStrings)
                .stream()
    ...
    private static List<String> splitMultipleVlan(List<String> vlanStrings) {
        List<String> finalList = new ArrayList<>();
        for (String vlanString : vlanStrings) {
            if (vlanString.contains("-")) {
                var list = Arrays.stream(vlanString.split("-"))
                        .map(Integer::parseInt)
                        .collect(Collectors.toList());
                var stream = IntStream.range(list.get(0), list.get(1) + 1);
                stream.forEach(i -> finalList.add(String.valueOf(i)));
            } else {
                finalList.add(vlanString);
            }
        }
        return finalList;
    }
