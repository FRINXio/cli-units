...
package io.frinx.cli.unit.sonic.init;
...
public class SonicCliInitializerUnit extends AbstractUnit {

    private static final Logger LOG = LoggerFactory.getLogger(SonicCliInitializerUnit.class);

    public SonicCliInitializerUnit(@NotNull final TranslationUnitCollector registry) {
        super(registry);
    }

    @Override
    protected Set<Device> getSupportedVersions() {
        return Collections.singleton(SonicDevices.SONIC_GENERIC);
    }

    @Override
    protected String getUnitName() {
        return "SONIC cli init (FRINX) translate unit";
    }
    ...
    public SessionInitializationStrategy getInitializer(@NotNull final RemoteDeviceId id,
        ...
        return new SonicCliInitializer(id);
    }
    ...
    /**
     * Initialize SONiC CLI session to be usable by various CRUD and RPC handlers.
     */
    public static class SonicCliInitializer implements SessionInitializationStrategy {
        ...
        public SonicCliInitializer(RemoteDeviceId id) {
            this.id = id;
        }
