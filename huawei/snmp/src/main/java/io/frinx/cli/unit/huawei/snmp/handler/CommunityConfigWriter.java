...
public class CommunityConfigWriter implements CliWriter<Config> {
    ...
    private static final String WRITE_TEMPLATE = "system-view\n"
            + "{% if ($config.local_engineid) %}"
            + "snmp-agent local-engineid {$config.local_engineid}\nY\n"
            + "{% else %}undo snmp-agent local-engineid\nY\n{% endif %}"
            + "{% if ($config.read_community_password) %}"
            + "snmp-agent community read {$config.read_community_password.plain_string.value} acl 2000\nY\n"
            + "{% else if($before.read_community_password) %}"
            + "undo snmp-agent community {$before.read_community_password.plain_string.value}\nY\n{% endif %}"
            + "{% if ($config.write_community_password) %}"
            + "snmp-agent community write {$config.write_community_password.plain_string.value} acl 2000\nY\n"
            + "{% else if($before.write_community_password) %}"
            + "undo snmp-agent community {$before.write_community_password.plain_string.value}\nY\n{% endif %}"
            + "{% if ($config.community_location) %}"
            + "snmp-agent sys-info location {$config.community_location}\nY\n"
            + "{% else %}undo snmp-agent sys-info location\nY\n{% endif %}"
            + "return";

    private static final String DELETE_TEMPLATE = "system-view\n"
            + "undo snmp-agent local-engineid\nY\n"
            + "{% if ($before.read_community_password) %}"
            + "undo snmp-agent community {$before.read_community_password.plain_string.value}\nY\n{% endif %}"
            + "{% if ($before.write_community_password) %}"
            + "undo snmp-agent community {$before.write_community_password.plain_string.value}\nY\n{% endif %}"
