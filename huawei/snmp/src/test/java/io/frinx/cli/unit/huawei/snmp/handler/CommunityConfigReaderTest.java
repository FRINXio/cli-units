...
class CommunityConfigReaderTest {
    ...
    private static final String SNMP_OUTPUT = """
             snmp-agent local-engineid 21315435DB030425C50419D4
             snmp-agent community read achzv%^y*}^kQ:mui12BC1k)-4t$1Fl}\\(_@4\\5@'SOKx%iM;B]x`NdJz#D}ka5-MiR]R$dYM'O+8BAJfN{J%^%# acl 2000\s
             snmp-agent community write %^%#0jwFL"`KeH('[_<CZ2R0oqa,![(p+M,l5@DCx0`3CDW3HRuitP0ili0F[udR2V2:%l_b)Ffawd63&^(*& acl 2000\s
             snmp-agent sys-info location Mukachevo  \s
             snmp-agent sys-info version v2c         \s
             snmp-agent trap source LoopBack0\s
            """;
    ...
    void testSNMPIds() {
        ...
        assertEquals(new ConfigBuilder().setLocalEngineid("21315435DB030425C50419D4")
