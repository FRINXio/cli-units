...
class AccountingSchemasReaderTest {

    private final String outputAccounts = """
             accounting-scheme default
             accounting-scheme ACC-SC
            """;

    @Test
    void testAccountingIds() {
        List<AccountKey> keys = AccountingSchemasReader.getAllIds(outputAccounts);
        assertEquals(keys, Arrays.asList(new AccountKey("default"), new AccountKey("ACC-SC")));
    }
}
