...
public class ClockTimezoneConfigWriter implements CliWriter<Config> {

    private static final String WRITE_CLOCK = "clock timezone "
            + "{$timezoneName} "
            + "{$timezoneModifier} "
            + "{$timezoneOffset}\n"
            + "return";
    ...
    public ClockTimezoneConfigWriter(Cli cli) {
        this.cli = cli;
    }
    ...
    public void deleteCurrentAttributes(@NotNull InstanceIdentifier<Config> instanceIdentifier,
        ...
        throw new DeleteFailedException(instanceIdentifier,
                new IllegalStateException("Deleting clock timezone is not permitted."));
    }

    @VisibleForTesting
    String writeTemplate(Config config) {
        return fT(WRITE_CLOCK, "timezoneName", config.getName(),
                "timezoneModifier", config.getModifier().getName(),
                "timezoneOffset", config.getOffset());
    }
}