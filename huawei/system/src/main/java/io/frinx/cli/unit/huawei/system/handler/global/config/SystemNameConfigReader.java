...
public final class SystemNameConfigReader implements CliConfigReader<Config, ConfigBuilder> {

    private static final String SYSTEM_NAME_INFO = "display current-configuration configuration global-config";
    private static final Pattern SYSTEM_NAME = Pattern.compile("\\s+sysname (?<name>.*)");
    private final Cli cli;

    public SystemNameConfigReader(Cli cli) {
        this.cli = cli;
    }
    ...
    public void readCurrentAttributes(@Nonnull InstanceIdentifier<Config> id,
        ...
        parseConfigAttributes(blockingRead(Command.showCommandNoCaching(SYSTEM_NAME_INFO), cli, id), configBuilder);
    }

    @VisibleForTesting
    static void parseConfigAttributes(String output, ConfigBuilder configBuilder) {
        ParsingUtils.NEWLINE.splitAsStream(output)
                .filter(value -> value.contains("sysname "))
                .map(SYSTEM_NAME::matcher)
                .filter(Matcher::matches)
                .map(m -> m.group("name"))
                .findFirst()
                .ifPresent(configBuilder::setSystemName);
    }
}