...
public class AclReader implements CliConfigListReader<AclSet, AclSetKey, AclSetBuilder> {

    private final Cli cli;
    private final AclEntryReader aclEntryReader;
    private final AclConfigReader aclConfigReader;
    public static final String SH_ACL_NAME = "display current-configuration | include acl name";
    public static final Pattern ACL_NAME_CHUNK = Pattern.compile("acl name (?<name>\\S+) (?<accessList>\\S+)");


    public AclReader(Cli cli) {
        this.cli = cli;
        this.aclEntryReader = new AclEntryReader(cli, this);
        this.aclConfigReader = new AclConfigReader(cli, this);
    }

    @NotNull
    @Override
    public List<AclSetKey> getAllIds(@NotNull InstanceIdentifier<AclSet> instanceIdentifier,
                                      @NotNull ReadContext readContext) throws ReadFailedException {
        return parseAclNames(blockingRead(SH_ACL_NAME, cli, instanceIdentifier, readContext));
    }

    @VisibleForTesting
    static List<AclSetKey> parseAclNames(@NotNull String aclSetConfiguration) {
        return ParsingUtils.parseFields(aclSetConfiguration, 0,
            ACL_NAME_CHUNK::matcher,
            matcher -> matcher.group("name"),
            aclSetKey -> new AclSetKey(aclSetKey, ACLIPV4.class));
    }
    ...
    public void readCurrentAttributes(@NotNull InstanceIdentifier<AclSet> instanceIdentifier,
                                      @NotNull AclSetBuilder aclNameBuilder, @NotNull ReadContext readContext)
        ...
        VrpAclSetAug vrpAclSetAug = getAclAugConfig(instanceIdentifier, readContext);

        aclNameBuilder.setName(aclName)
                .setType(aclType)
                .setAclEntries(aclEntries)
                .setConfig(getAclConfig(aclSetKey, vrpAclSetAug));
    }

    private static Config getAclConfig(final AclSetKey aclSetKey, final VrpAclSetAug vrpAclSetAugBuilder) {
        return new ConfigBuilder()
                .setName(aclSetKey.getName())
                .setType(aclSetKey.getType())
                .addAugmentation(VrpAclSetAug.class, vrpAclSetAugBuilder)
                .build();
    }
    ...
    private VrpAclSetAug getAclAugConfig(@NotNull InstanceIdentifier<AclSet> id, @NotNull ReadContext ctx)
            throws ReadFailedException {
        final VrpAclSetAugBuilder vrpAclSetAugBuilder = new VrpAclSetAugBuilder();
        aclConfigReader.readCurrentAttributes(id, vrpAclSetAugBuilder, ctx);
        return vrpAclSetAugBuilder.build();
    }
}