...
public class OneRateTwoColorWriter implements CliWriter<OneRateTwoColor> {
    ...
    private static final String TEMPLATE = "system-view\n"
            ...
            + "{% if ($config) %}car cir"
            + "{% if ($config.cir_pct) %} pct {$config.cir_pct.value} {% endif %}"
            + "{% if ($aug_hua.color_mode) %}mode {$aug_hua.color_mode.name}{% endif %}"
            + "{% if ($conform_action) %} {$conform_action}{% endif %}"
            + "{% if ($yellowAction) %} {$yellowAction}{% endif %}"
            + "{% if ($exceed_action) %} {$exceed_action}\n{% endif %}"
            + "{% else %}undo car cir\n{% endif %}"
            + "return";
