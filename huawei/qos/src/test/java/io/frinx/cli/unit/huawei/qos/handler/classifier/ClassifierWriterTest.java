...
public class ClassifierWriterTest {
    ...
    private static final String WRITE_ALL = "system-view\n"
            + "traffic classifier map1 operator or\n"
            + "if-match dscp cs3\n"
            + "return\n";

    private static final String WRITE_MORE = "system-view\n"
            + "traffic classifier map1 operator or\n"
            + "if-match dscp 13 12\n"
            + "return\n";
    ...
    public void writeAny() throws WriteFailedException {
        ...
        writer.writeCurrentAttributes(piid, data, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        Assert.assertEquals(WRITE_MORE, response.getValue().getContent());
    }
