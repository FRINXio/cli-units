...
public class SchedulerConfigReaderTest {
    ...
    public void testServicePolicy() {
        ...
        SchedulerConfigReader.fillInConfig(OUTPUT, configBuilder);
        VrpQosSchedulerConfAug aug = configBuilder.getAugmentation(VrpQosSchedulerConfAug.class);
        Assert.assertEquals("10", aug.getVrpPrecedence());
        Assert.assertEquals("VIDEO", aug.getBehavior());
    }
}
