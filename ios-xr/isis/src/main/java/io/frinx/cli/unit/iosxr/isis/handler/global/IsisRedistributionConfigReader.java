...
public class IsisRedistributionConfigReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    private LevelType convertRedistLevelFromString(String level) {
        ...
        return switch (level) {
            case "level-1" -> LevelType.LEVEL1;
            case "level-2" -> LevelType.LEVEL2;
            case "level-1-2" -> LevelType.LEVEL12;
            default -> throw new IllegalArgumentException("Unknown redistribution level " + level);
        };
    }
