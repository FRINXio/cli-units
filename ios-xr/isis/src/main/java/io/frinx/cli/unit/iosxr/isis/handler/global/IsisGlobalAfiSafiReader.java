...
public class IsisGlobalAfiSafiReader implements CliConfigListReader<Af, AfKey, AfBuilder> {
    ...
    public List<AfKey> getAllIds(
        @NotNull InstanceIdentifier<Af> id,
    ...
    private static Class<? extends AFITYPE> convertAfiTypeFromString(String afi) {
        return switch (afi) {
            case "ipv4" -> IPV4.class;
            case "ipv6" -> IPV6.class;
            default -> throw new IllegalArgumentException("Unknown AFI type " + afi);
        };
    }

    private static Class<? extends SAFITYPE> convertSafiTypeFromString(String safi) {
        return switch (safi) {
            case "unicast" -> UNICAST.class;
            case "multicast" -> MULTICAST.class;
            default -> throw new IllegalArgumentException("Unknown SAFI type " + safi);
        };
    }
