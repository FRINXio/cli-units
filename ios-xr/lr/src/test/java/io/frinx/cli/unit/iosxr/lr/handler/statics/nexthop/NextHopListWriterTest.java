...
class NextHopListWriterTest {
    ...
    @Test
    void testWriteCurrentAttributesWResult() throws WriteFailedException {
        ...
        assertEquals(WRITE_COMMAND, response.getValue().getContent());
    }

    @Test
    void testDeleteCurrentAttributesWResult() throws WriteFailedException {
        ...
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(DELETE_COMMAND, response.getValue().getContent());
    }
}
