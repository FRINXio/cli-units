...
public class StubRouterConfigWriter implements CliWriter<Config> {
    ...
    @Override
    public void writeCurrentAttributes(@NotNull InstanceIdentifier<Config> iid, @NotNull Config data,
    ...
    public void updateCurrentAttributes(@NotNull InstanceIdentifier<Config> iid, @NotNull Config dataBefore,
                                        @NotNull Config dataAfter, @NotNull WriteContext context) throws
    ...
    @Override
    public void deleteCurrentAttributes(@NotNull InstanceIdentifier<Config> iid, @NotNull Config data,
                                        @NotNull WriteContext context) throws WriteFailedException {
        final String processName = iid.firstKeyOf(Protocol.class).getName();
        final String nwInsName = OspfV3ProtocolReader.resolveVrfWithName(iid);
        blockingWriteAndRead(cli, iid, data,
