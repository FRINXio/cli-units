...
public class MemberConfigReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    static void parseLacpConfig(@NotNull ConfigBuilder configBuilder, String ifcName, String interfaceConfiguration) {
    ...
    private static AtomicReference<LacpActivityType> parseLacpActivityType(@NotNull String interfaceConfiguration) {
        ...
        ParsingUtils.parseField(interfaceConfiguration,
                LACP_BUNDLE_AND_MODE_LINE::matcher, matcher -> matcher.group("mode"), mode ->
                        fetchedMode.set(LacpActivityType.valueOf(mode.toUpperCase(Locale.ROOT))));
        return fetchedMode;
    }

    static AtomicReference<LacpPeriodType> parseLacpPeriodType(@NotNull String interfaceConfiguration) {
