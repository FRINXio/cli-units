...
public class BundleConfigReader implements CliConfigReader<Config, ConfigBuilder> {
    ...
    static void readConfiguration(@NotNull String bundleName, @NotNull String interfaceConfiguration,
    ...
    private static LacpActivityType parseLacpMode(@NotNull String interfaceConfiguration) {
        return ParsingUtils.parseField(
                interfaceConfiguration,
                0,
                LACP_MODE_LINE::matcher,
            matcher -> matcher.group("mode"))
                .map(mode -> LacpActivityType.valueOf(mode.toUpperCase(Locale.ROOT)))
                .orElse(null);
    }
}