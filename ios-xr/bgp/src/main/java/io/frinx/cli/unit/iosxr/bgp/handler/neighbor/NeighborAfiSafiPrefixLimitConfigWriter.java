...
public class NeighborAfiSafiPrefixLimitConfigWriter implements CliWriter<Config> {
    ...
    static final String NEIGHBOR_AFI_PREFIX_LIMIT = """
            router bgp {$as} {$instance} {$vrf}
            neighbor {$address}
            address-family {$afiSafi}
            {.if ($config.max_prefixes) }maximum-prefix {$config.max_prefixes}{.else}no maximum-prefix{/if}{.if ($config.shutdown_threshold_pct.value) } {$config.shutdown_threshold_pct.value}{/if}
            root
            """;

    static final String DELETE_NEIGHBOR_AFI_PREFIX_LIMIT = """
