...
class PeerGroupAfiSafiApplyPolicyConfigWriterTest {

    private static final String WRITE_INPUT = """
            router bgp 17676 instance instance1 vrf vrf1
    ...
    private static final String WRITE_INPUT2 = """
            router bgp 17676 instance instance1 vrf vrf1
            neighbor-group PEER-GROUP01
            address-family vpnv4 unicast
            route-policy POLICY_IN in
    ...
    @Test
    void testWriterCurrentAttributes01() throws WriteFailedException {
    ...
    @Test
    void testWriterCurrentAttributes02() throws WriteFailedException {
        Config config = new ConfigBuilder()
                .setImportPolicy(Collections.singletonList(POLICY_IN))
                .build();
        target.writeCurrentAttributes(iid, config, context);
        Mockito.verify(cli).executeAndRead(response.capture());
        assertEquals(WRITE_INPUT2, response.getValue().getContent());
    }
