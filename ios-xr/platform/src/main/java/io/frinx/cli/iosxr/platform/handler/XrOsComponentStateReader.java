...
public class XrOsComponentStateReader implements CliOperReader<State, StateBuilder> {
    ...
    static void parseFields(@Nonnull StateBuilder stateBuilder, String name, String output) {
        ...
        for (Set<String> typeNames : HW_TYPES.keySet()) {
            Class<? extends OPENCONFIGHARDWARECOMPONENT> candidateType = HW_TYPES.get(typeNames);

            for (String typeName : typeNames) {
                if (stateBuilder.getDescription().toLowerCase().contains(typeName)) {
