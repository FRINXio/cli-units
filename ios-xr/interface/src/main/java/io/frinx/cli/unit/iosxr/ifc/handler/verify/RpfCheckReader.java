...
public class RpfCheckReader implements CliConfigReader<VerifyUnicastSourceReachableVia,
    ...
    @Override
    public void readCurrentAttributes(@NotNull final InstanceIdentifier<VerifyUnicastSourceReachableVia>
                                                  instanceIdentifier,
                                      @NotNull final VerifyUnicastSourceReachableViaBuilder builder,
    ...
    private ChildOf<? extends DataObject> configFromMatcher(final Matcher matcher) {
        ...
        if ("ipv4".equalsIgnoreCase(type)) {
            return new Ipv4Builder()
                    .setRpfCheck(RpfCheck.valueOf(rpfConfig.toUpperCase(Locale.ROOT)))
