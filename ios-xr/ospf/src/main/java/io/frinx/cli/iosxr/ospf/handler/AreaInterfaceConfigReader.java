...
import org.opendaylight.yang.gen.v1.http.openconfig.net.yang.ospf.types.rev170228.OspfMetric;
...
public class AreaInterfaceConfigReader implements OspfReader.OspfConfigReader<Config, ConfigBuilder> {
    ...
    private static final String SHOW_OSPF_INT = "sh ip ospf interface %s";
    private static final Pattern COST_LINE = Pattern.compile("Process ID (?<pid>[^,]+), Router ID (?<routerId>[^,]+), Network Type (?<type>[^,]+), Cost: (?<cost>.+)");
    ...
    public void readCurrentAttributesForType(@Nonnull InstanceIdentifier<Config> instanceIdentifier, @Nonnull ConfigBuilder configBuilder, @Nonnull ReadContext readContext) throws ReadFailedException {
        ...
        String output = blockingRead(String.format(SHOW_OSPF_INT, key.getId()), cli, instanceIdentifier, readContext);
