...
public class ClassifierWriter implements CliWriter<Classifier> {
    ...
    private static final String MATCH_IPV4_PREC_T ="{% if ($v4Aug.precedences) %}" +
        "match precedence ipv4 {% loop in $v4Aug.precedences as $prec %}" + LIST_PREC + "{% endif %}";

    private static final String MATCH_IPV6_PREC_T ="{% if ($v6Aug.precedences) %}" +
        "match precedence ipv6 {% loop in $v6Aug.precedences as $prec %}" + LIST_PREC + "{% endif %}";
    ...
    private void writeMatch(InstanceIdentifier<Classifier> instanceIdentifier, Classifier classifier, String className, Conditions conditions, ClassMapType type) throws WriteFailedException.CreateFailedException {
    ...
    private void writeMatchAny(InstanceIdentifier<Classifier> instanceIdentifier, Classifier classifier, String className, List<Term> terms) throws WriteFailedException.CreateFailedException {
    ...
    private QosIpv4ConditionAug findIpv4Aug(Conditions c) {
        if (c.getIpv4() != null) {
            org.opendaylight.yang.gen.v1.http.frinx.openconfig.net.yang.header.fields.rev171215.ipv4.protocol.fields.top.ipv4.Config v4Config =
                c.getIpv4().getConfig();
    ...
    private QosIpv6ConditionAug findIpv6Aug(Conditions c) {
        if (c.getIpv6() != null) {
            org.opendaylight.yang.gen.v1.http.frinx.openconfig.net.yang.header.fields.rev171215.ipv6.protocol.fields.top.ipv6.Config v6Config =
                c.getIpv6().getConfig();
    ...
    @Override
    public void updateCurrentAttributes(@Nonnull InstanceIdentifier<Classifier> id, @Nonnull Classifier dataBefore, @Nonnull Classifier dataAfter, @Nonnull WriteContext writeContext) throws WriteFailedException {
        deleteCurrentAttributes(id, dataBefore, writeContext);
        writeCurrentAttributes(id, dataAfter, writeContext);
    }

    @Override
    public void deleteCurrentAttributes(@Nonnull InstanceIdentifier<Classifier> instanceIdentifier, @Nonnull Classifier classifier, @Nonnull WriteContext writeContext) throws WriteFailedException {
        final String className = instanceIdentifier.firstKeyOf(Classifier.class).getName();
