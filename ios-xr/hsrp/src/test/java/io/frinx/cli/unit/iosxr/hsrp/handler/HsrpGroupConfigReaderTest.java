...
public class HsrpGroupConfigReaderTest {
    ...
    public void testReadCurrentAttributes_001() throws Exception {
        ...
        // verify
        Assert.assertEquals(builder.getAddressFamily(), HsrpUtil.getType(familyType));
        Assert.assertEquals(builder.getVirtualRouterId(), virtualRouterId);
        Assert.assertEquals(builder.getVersion(), HsrpGroupConfigReader.DEFAULT_VERSION);
        Assert.assertEquals(builder.getPriority(), priority);
    }
}