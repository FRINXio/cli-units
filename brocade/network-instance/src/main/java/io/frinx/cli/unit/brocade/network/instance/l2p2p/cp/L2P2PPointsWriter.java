...
public class L2P2PPointsWriter implements CompositeWriter.Child<ConnectionPoints>, CliWriter<ConnectionPoints> {
    ...
    static String VLL_REMOTE = """
            configure terminal
            router mpls
            vll {$network} {$remote.remote.config.virtual_circuit_identifier}
            {% if ($mtu) %}vll-mtu {$mtu}
            {% endif %}vll-peer {$remote.remote.config.remote_system.ipv4_address.value}
            end
            """;

    static String VLL_IFC = """
            configure terminal
            router mpls
            vll {$network} {$remote.remote.config.virtual_circuit_identifier}
    ...
    static String VLL_SUBINTERFACE = """
            configure terminal
            router mpls
            vll {$network} {$remote.remote.config.virtual_circuit_identifier}
    ...
    static String DELETE_VLL_LOCAL = """
            configure terminal
            router mpls
    ...
    static String VLL_LOCAL_SUBINTERFACE = """
            configure terminal
            router mpls
            vll-local {$network}
            vlan {$local.local.config.subinterface}
