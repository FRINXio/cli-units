...
public class L2VSIVlanReaderTest {
    ...
    public void parseVlans() {
        List<VlanKey> vlanKeys = L2VSIVlanReader.parseVlans(OUTPUT, "Blizznet-WLAN-User-200");
        Assert.assertEquals(2, vlanKeys.size());

        vlanKeys = L2VSIVlanReader.parseVlans(OUTPUT, "FRINXTEST666");
        Assert.assertEquals(1, vlanKeys.size());

        vlanKeys = L2VSIVlanReader.parseVlans(OUTPUT, "WBNWLC-Management");
        Assert.assertEquals(0, vlanKeys.size());

        vlanKeys = L2VSIVlanReader.parseVlans(OUTPUT_EMPTY, "aaa");
        Assert.assertEquals(0, vlanKeys.size());
    }
}