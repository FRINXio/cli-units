...
package io.frinx.cli.nexus.conf.handler;
...
public class ConfigMetadataReader implements CliOperReader<ConfigurationMetadata, ConfigurationMetadataBuilder> {
    ...
    private static final String SHOW_LAST_COMMIT_TIME = "show logging | include \"Configured from\"";
    private static final String DATE_REGEX = "(?<time>\\d.+) (?<device>.+) (?<config>%.+): Configured from"
            + " (?<user>.+).*";
