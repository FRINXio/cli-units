...
public class EthernetConfigWriterTest {
    ...
    private static final String WRITE_INPUT = "interface Ethernet1/1\n"
            + "channel-group 6 mode active\n"
            + "root\n";

    private static final String UPDATE_INPUT = "interface Ethernet1/1\n"
            + "channel-group 6 mode passive\n"
            + "root\n";

    private static final String UPDATE_ON = "interface Ethernet1/1\n"
            + "channel-group 7\n"
            + "root\n";

    private static final String DELETE_INPUT = "interface Ethernet1/1\n"
            + "no channel-group\n"
            + "root\n";
    ...
    @Test
    public void updateOn() throws WriteFailedException {
        // clean what we can
        Config newData = new ConfigBuilder().addAugmentation(Config1.class,
        ...
        Mockito.verify(cli)
                .executeAndRead(response.capture());
        Assert.assertEquals(UPDATE_ON, response.getValue()
                .getContent());
    }
